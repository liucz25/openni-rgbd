...
int main(int argc, char *argv[])
{
	...
	while (bExit)
	{
		...
		cv::Mat cv_rgbmerge(IMAGE_HEIGHT_480, IMAGE_WIDTH_640, CV_8UC3);
		...
		{	int ret = 0;
			...
			mDraw.ShowImagesSideBySide(_winTitle, imgsTwo, "", 10, 450);
		...
		if (0x33 == char(c))//3
		{
			bCapture=true;
			//nRet = pD2CSwapper->StreamStop();
			printf("ssssssssssssssssssssssssssssssssssssssssssstop stream :%d\n", nRet);
			//d2cType = NORMAL_NO;
			//pD2CSwapper->SetD2CType(NORMAL_NO);
		}
		if (0x34 == char(c))//4
		{
			bCapture=false;
			//nRet = pD2CSwapper->StreamStop();
			printf("stttttttttttttttttttttttttttttttttttttttttttttttttttttttop stream :%d\n", nRet);
			//d2cType = NORMAL_NO;
			//pD2CSwapper->SetD2CType(NORMAL_NO);
		}
	}
